...
function probeProfile(snapshot) {
...
function createBinding(snapshot, probe, receiptBytes, cliArgs, exitCode) {
  const payload = {
...
function verifyBinding(binding, profileBytes, receiptBytes) {
  const errors = [];
...
function planProfileCommand(snapshot, args, probe) {
...
function registerProfileTools(server, api) {
...
registerProfileTools(server,{z,fs,path,loadProfile,probeProfile,createBinding,planProfileCommand,assertAllowedPath,assertWorkspacePath,resolveLabRoot,invokeCli});
  }
  return server;
}
